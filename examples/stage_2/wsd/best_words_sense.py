...
def get_top_synsets(graph, number=3):
    graph.count_len()
...
sentence = "I worked all day on farm and all day on factory.".lower()
word = "worked"
word_order = 0

lemmas_set, start_word = sent_processing.get_sen_lemmas(sentence, word, word_order)
tokens_dict = sent_processing.get_pos_tokens(sentence)
graph = build_word_graph(lemmas_set, start_word)
synsets = get_top_synsets(graph)

for synset in synsets:
    print(synset.name(), synset.definition(), synset.examples()[0])
