...
def get_wn_definitions(sentence, word, word_order):
    ...
    graph = graph_wsd.build_word_graph(lemmas_set, start_word)
    synsets = graph_wsd.get_top_synsets(graph, start_word)
    word_definitions = []
...
def get_definitions(sentence, word, word_order):
    senses = []
    tokens_dict = auxiliar_functions.get_pos_tokens(sentence)
...
if __name__ == "__main__":
    sentence = "I worked all day on the farm or all day on factory.".lower()
    word = "worked"
    word_order = 0
